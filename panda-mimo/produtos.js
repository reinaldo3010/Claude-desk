/* Cópia do catálogo que vem junto com o site.
   Serve de reserva quando o banco não responde e como estado inicial da página.
   O painel gera uma versão nova deste arquivo em Configurações. */
window.PANDA_PRODUTOS = [
  {
    "slug": "garrafas-termicas",
    "nome": "Garrafas térmicas",
    "descricao": "Aço inox 304 com tampa rosqueável e alça. Mantém 24h gelada e 12h quente. Perfeita pra treino, trabalho e escola.",
    "etiquetas": [
      "1 litro",
      "24h gelada · 12h quente",
      "Livre de BPA"
    ],
    "mensagem": "Oi, Panda Mimo! Quero uma garrafa térmica personalizada 🐼",
    "rotulo_botao": "Quero essa",
    "em_breve": false,
    "fotos": [
      {
        "url": "assets/prod-garrafa.webp",
        "alt": "Garrafa térmica Panda Mimo de 1 litro, frente com o logo",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-garrafa-frase.webp",
        "alt": "Garrafa térmica com a frase: mais que um presente, um mimo",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-garrafa-panda.webp",
        "alt": "Garrafa térmica com o mascote panda segurando uma garrafinha",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-garrafa-interior.webp",
        "alt": "Garrafa térmica aberta, mostrando o interior em aço inox",
        "largura": 760,
        "altura": 760
      }
    ],
    "ordem": 0
  },
  {
    "slug": "canecas",
    "nome": "Canecas",
    "descricao": "Porcelana premium com o seu nome ou a sua frase. O café da manhã fica mais gostoso quando a caneca é sua.",
    "etiquetas": [
      "325 ml",
      "Porcelana premium",
      "Nome ou frase"
    ],
    "mensagem": "Oi, Panda Mimo! Quero uma caneca personalizada 🐼",
    "rotulo_botao": "Quero essa",
    "em_breve": false,
    "fotos": [
      {
        "url": "assets/prod-caneca.webp",
        "alt": "Caneca de porcelana branca personalizada com o nome Malu",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-caneca-rosa.webp",
        "alt": "Caneca com interior e alça rosa sobre uma tábua de madeira",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-caneca-lado.webp",
        "alt": "Caneca inclinada, mostrando a estampa lateral",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-caneca-nomes.webp",
        "alt": "Três canecas personalizadas com os nomes Lívia, Carlos e Ana",
        "largura": 760,
        "altura": 760
      }
    ],
    "ordem": 1
  },
  {
    "slug": "copos-termicos",
    "nome": "Copos térmicos",
    "descricao": "Tampa hermética com bico e base antiderrapante. O queridinho de quem vive de café gelado, chá ou água com limão.",
    "etiquetas": [
      "850 ml",
      "24h gelado · 12h quente",
      "Livre de BPA"
    ],
    "mensagem": "Oi, Panda Mimo! Quero um copo térmico personalizado 🐼",
    "rotulo_botao": "Quero esse",
    "em_breve": false,
    "fotos": [
      {
        "url": "assets/prod-copo.webp",
        "alt": "Copo térmico Panda Mimo de 850 ml, frente com o logo",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-copo-lado.webp",
        "alt": "Copo térmico visto de lado, com a estampa de flores",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-copo-costas.webp",
        "alt": "Copo térmico com a frase: feito com carinho, feito pra você",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-copo-nomes.webp",
        "alt": "Três copos térmicos personalizados com nomes diferentes",
        "largura": 760,
        "altura": 760
      }
    ],
    "ordem": 2
  },
  {
    "slug": "ecobags",
    "nome": "Ecobags",
    "descricao": "100% algodão cru, alça reforçada e interior amplo. Leve o seu mimo pra escola, pro trabalho e pras compras.",
    "etiquetas": [
      "100% algodão",
      "Alça reforçada",
      "Lavável"
    ],
    "mensagem": "Oi, Panda Mimo! Quero uma ecobag personalizada 🐼",
    "rotulo_botao": "Quero essa",
    "em_breve": false,
    "fotos": [
      {
        "url": "assets/prod-ecobag-costas.webp",
        "alt": "Ecobag de algodão com a frase: mais que um presente, um mimo",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-ecobag-lado.webp",
        "alt": "Ecobag de algodão vista de lado, com a estampa do panda",
        "largura": 760,
        "altura": 760
      }
    ],
    "ordem": 3
  },
  {
    "slug": "tags-e-chaveiros",
    "nome": "Tags e chaveiros",
    "descricao": "Peças redondas em MDF ou acrílico com cordão. Ótimas pra lembrancinhas, mochilas e presentes de turma.",
    "etiquetas": [
      "5 cm",
      "A partir de 10 unid.",
      "Cordão incluso"
    ],
    "mensagem": "Oi, Panda Mimo! Quero tags ou chaveiros personalizados 🐼",
    "rotulo_botao": "Quero essa",
    "em_breve": false,
    "fotos": [
      {
        "url": "assets/prod-tag.webp",
        "alt": "Tag redonda de MDF com cordão e o logo Panda Mimo",
        "largura": 760,
        "altura": 760
      }
    ],
    "ordem": 4
  },
  {
    "slug": "kits-presenteaveis",
    "nome": "Kits presenteáveis",
    "descricao": "Monte um conjunto: garrafa + caneca + tag. A gente embala em caixa kraft com fita e bilhetinho. Chega pronto pra entregar.",
    "etiquetas": [
      "Combinação livre",
      "Bilhete incluso",
      "Caixa kraft"
    ],
    "mensagem": "Oi, Panda Mimo! Quero montar um kit presenteável 🐼",
    "rotulo_botao": "Quero esse",
    "em_breve": false,
    "fotos": [
      {
        "url": "assets/prod-kit.webp",
        "alt": "Caixa de presente com fita e caneca personalizada Panda Mimo",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-kit-kraft.webp",
        "alt": "Caixa kraft com fita e o logo Panda Mimo",
        "largura": 760,
        "altura": 760
      },
      {
        "url": "assets/prod-kit-caixa.webp",
        "alt": "Caixa individual da garrafa térmica Panda Mimo",
        "largura": 760,
        "altura": 760
      }
    ],
    "ordem": 5
  },
  {
    "slug": "novidades-em-breve",
    "nome": "Novidades em breve",
    "descricao": "Estamos testando peças novas. Quer ser avisado quando saírem? A gente manda uma mensagem, sem spam.",
    "etiquetas": [
      "Lista de espera",
      "Sem compromisso"
    ],
    "mensagem": "Oi, Panda Mimo! Quero entrar na lista pra saber das novidades 🐼",
    "rotulo_botao": "Me avise",
    "em_breve": true,
    "fotos": [
      {
        "url": "assets/panda-novidades.webp",
        "alt": "Mascote Panda Mimo segurando uma placa: novidades em breve",
        "largura": 805,
        "altura": 900
      }
    ],
    "ordem": 6
  }
];
