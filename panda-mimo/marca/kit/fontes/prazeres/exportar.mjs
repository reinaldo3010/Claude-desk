...
import { ILUSTRACOES_HOBBIES, MODELOS_HOBBIES } from '../../../../simulador/hobbies.js';
import { PALETA } from '../../../../simulador/paleta.js';

const destino = new URL('../../svg/prazeres/', import.meta.url);
await mkdir(destino, { recursive: true });
for (const [id, spec] of Object.entries(ILUSTRACOES_HOBBIES)) {
...
await writeFile(new URL('modelos.json',destino), JSON.stringify(MODELOS_HOBBIES,null,2));
console.log(`${Object.keys(ILUSTRACOES_HOBBIES).length} SVGs e ${MODELOS_HOBBIES.length} modelos exportados.`);
