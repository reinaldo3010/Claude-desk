...
import mascote as M, coracoes as C
...
def sim(fill): return (120,60,980,1140), M.simbolo(fill)
variantes('simbolo',sim,60)
# 4) logotipo vertical: símbolo (~48% da largura do letreiro) em cima do letreiro
def vert(fill):
    sw=980; esc_s=(1818*0.48)/sw; alt_s=1140*esc_s
    c=f'<g transform="translate({(1818-sw*esc_s)/2:.1f} 0) scale({esc_s:.4f}) translate(-120 -60)">{M.simbolo(fill)}</g>'
...
c=f'<circle cx="600" cy="600" r="600" fill="{COR["oliva"]}"/><g transform="translate(600 640) scale(0.78) translate(-610 -630)">{M.simbolo(COR["offwhite"],20,coracao_solto=False)}</g>'
salva('avatar', svg((0,0,1200,1200),c,'Avatar Seu Mimo Studio'))
c=f'<circle cx="600" cy="600" r="600" fill="{COR["offwhite"]}"/><g transform="translate(600 640) scale(0.78) translate(-610 -630)">{M.simbolo(COR["oliva"],20,coracao_solto=False)}</g>'
...
for nome,p in M.POSES.items():
    corpo=M.mascote(**p)
    salva(f'mascote-{nome}', svg((60,80,1180,1120),f'<g id="mascote-{nome}">{corpo}</g>',f'Mascote Seu Mimo Studio, {nome}'))
    salva(f'mascote-{nome}-fundo', svg((60,80,1180,1120),f'<g id="mascote-{nome}">{corpo}</g>',f'Mascote Seu Mimo Studio, {nome}',COR['fundo']))
# 7) expressões: rosto em círculo (recorte da caixa, sem braços)
for nome,(oe,od,bc) in M.EXPRESSOES.items():
    corpo=M.mascote(pose='feliz',olhos=(oe,od),boca_t=bc,item='nenhum',etiqueta=False,bochechas=(nome not in ('surpreso','determinado')))
    c=f'<defs><clipPath id="cp"><circle cx="560" cy="560" r="330"/></clipPath></defs><circle cx="560" cy="560" r="330" fill="{COR["fundo"]}"/><g clip-path="url(#cp)"><g transform="translate(560 560) scale(1.15) translate(-560 -637)">{corpo}</g></g>'
...
for i,(nome,(oe,od,bc)) in enumerate(M.EXPRESSOES.items()):
    x=120+i*560; corpo=M.mascote(pose='feliz',olhos=(oe,od),boca_t=bc,item='nenhum',etiqueta=False,bochechas=(nome not in ('surpreso','determinado')))
    c+=f'<defs><clipPath id="cp{i}"><circle cx="{x+220}" cy="1620" r="200"/></clipPath></defs><circle cx="{x+220}" cy="1620" r="200" fill="{COR["offwhite"]}"/><g clip-path="url(#cp{i})"><g transform="translate({x+220} 1620) scale(0.7) translate(-560 -637)">{corpo}</g></g>'+rotulo(x+220,1900,nome)
c+=f'<g transform="translate(3760 1330) scale(0.46) translate(-120 -60)">{M.simbolo(COR["oliva"])}</g>'+rotulo(3985,1900,'símbolo')
c+=f'<text x="120" y="2150" font-family="Montserrat, Arial, sans-serif" font-size="30" fill="{COR["cafe"]}">Cores: kraft {M.COR["kraft"]} · lateral {M.COR["kraft_esc"]} · tampa {M.COR["kraft_topo"]} · fita {M.COR["oliva"]} · membros {M.COR["cafe"]} · etiqueta {M.COR["off"]} · bochechas {M.COR["rosa"]}</text>'
