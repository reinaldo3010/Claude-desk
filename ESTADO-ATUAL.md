...
**A galeria de artes prontas — 23/09/2026 (publicada em 23/09/2026, merge 3bfb036):** a pedido do dono ("criei umas 100 artes de caneca... a galeria vai chegar a milhares"; "como meu especialista, decida"). Decisão: as artes moram no Supabase que o site já usa (tabelas `pm_arte_categorias` e `pm_artes`, bucket `panda-mimo`, migração `pm_galeria_de_artes` já aplicada no banco), não no repositório. O painel ganhou a aba **Artes prontas** (sobe, prepara em 2480 × 1063 e miniatura, cria categoria, troca, tira do ar, apaga) e o estúdio mostra as artes no cardápio, depois dos modelos da casa, pelo caminho da arte do Canva. Guardião: bloco `galeria` com banco de mentira, galeria vazia nos outros blocos, quatro provas de defeito reprovadas. O primeiro lote (`Downloads/Imagens Panda Mimo`, 120 arquivos, 102 artes) foi lido uma a uma: **52 prontas**, 12 a consertar, 17 com texto errado, 21 com personagem licenciado, 18 cópias, separadas em subpastas (nada apagado). No ar e conferido com navegador (painel com a aba nova; estúdio lendo a galeria do banco, sem erro; caneca, garrafa e ecobag como antes). As categorias novas de criança e Fé ficaram em Momentos (escolha do dono): em grupos próprios o cardápio rolava por dentro em 1366 × 768. **As 52 artes subiram em 24/09/2026 e estão no ar** (conferido com navegador em 1280 e 390: "Todos os modelos" com 251, Natal com 21, a arte escolhida na caneca e no pedido, sem erro). O painel nunca teve login: o Authentication do Supabase não tinha nenhum usuário, então ninguém conseguia entrar. A pedido do dono, a subida foi por uma função temporária (`sobe-artes-da-galeria`), protegida por chave de uso único e desligada logo depois (hoje recusa tudo; pode ser apagada no painel do Supabase, em Edge Functions). Para as próximas artes, o dono cria o login dele (Authentication → Users → Add user, com o e-mail de `pm_admins`, marcando para confirmar) e sobe pela aba Artes prontas, em <https://reinaldo3010.github.io/Claude-desk/admin.html>. Leia `panda-mimo/simulador/GALERIA-DE-ARTES.md`. Fica com o dono: criar o login do painel, o nome Panda Mimo que a IA desenhou em 25 delas, o Pandinha 3D na arte de volta inteira (6.5) e a resolução dos originais (232 dpi, ampliados para 300).
...
**Publicado em 23/09/2026 (merge `3bfb036`):** a galeria de artes prontas (painel e estúdio), vazia até o dono subir as artes; ver a entrada do topo.
